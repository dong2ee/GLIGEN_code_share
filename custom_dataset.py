...
def clean_annotations(annotations):
    for anno_info in annotations:
        ...
        anno_info.pop(
            "category_id", None
        )  # I have checked that all 1 for flickr vg. This is not always 1 for coco, but I do not think we need this annotation
...
def make_a_sentence(obj_names, clean=False):
    ...
    for obj_name in obj_names:
        ...
        end_len = len(caption)
        caption += ", "
        tokens_positive.append([[start_len, end_len]])  # in real caption, positive tokens can be disjoint, thus using list of list
...
class CustomDataset(BaseDataset):
    def __init__(
        ...
        ## get image file list
        self.img_file = sorted(list(Path(self.dataset_path).rglob("*.jpg")))
    ...
    def get_item_from_dataset(self, index):
        item = {}
        item["data_id"] = index
        item["image"] = Image.open(str(self.img_file[index])).convert("RGB").resize((self.image_size, self.image_size))  # RGB
        item["annos"] = []

        ## embedding load
        embedding_path = str(self.img_file[index]).replace(".jpg", ".pk")
        if Path(embedding_path).exists():
            with open(embedding_path, "rb") as f:
                embedding_list = pickle.load(f)

        ## label load
        txt_path = str(self.img_file[index]).replace(".jpg", ".txt")
        if Path(txt_path).exists():
            with open(txt_path, "r") as f:
                read_lines = f.readlines()
                
                ## each bbox
                for idx, r_line in enumerate(read_lines):
                    label_line = r_line.split()

                    ## if class in CLASS_NAME
                    if int(label_line[0]) in CLASS_NAME.keys():
                        bbox = {}

                        ## bbox coords
                        b = list(map(float, r_line.split()[1:]))
                        x1, y1 = int((b[0] - b[2] / 2) * self.image_size), int((b[1] - b[3] / 2) * self.image_size)
                        b_w, b_h = int(b[2] * self.image_size), int(b[3] * self.image_size)
                        bbox["bbox"] = [x1, y1, b_w, b_h]

                        ## class name
                        bbox["category_name"] = CLASS_NAME[int(label_line[0])]

                        ## embeddings
                        bbox["text_embedding_before"] = embedding_list["text_embedding_before"][idx].detach().cpu().float()
                        #bbox["text_embedding_after"] = embedding_list["text_embedding_after"][idx].detach().cpu().float()
                        bbox["image_embedding_after"] = embedding_list["image_embedding_after"][idx].detach().cpu().float()

                        item["annos"].append(bbox)
                        
        ## caption load
        caption_path = str(self.img_file[index]).replace("jpg", "txt")
        with open(caption_path, "r") as f:
            item["caption"] = f.readline().strip("\n")
    ...
    def __getitem__(self, index):
        ...
        raw_item = self.get_item_from_dataset(index)
        out = {}
        ...
        out["path"] = str(self.img_file[index])
